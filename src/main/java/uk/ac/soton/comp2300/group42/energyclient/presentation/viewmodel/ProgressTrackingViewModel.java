...
public class ProgressTrackingViewModel {
    ...
    private final ObservableList<DataPoint> electricityData = FXCollections.observableArrayList();
    private final ObservableList<DataPoint> gasData = FXCollections.observableArrayList();
    ...
    private void loadAllChartDataAsync() {
        ...
        loadMockExpensesAsync();
        loadExpensesByCategoryAsync(EnergyCategory.ELECTRICITY, electricityData);
        loadExpensesByCategoryAsync(EnergyCategory.GAS, gasData);
        loadExpensesByCategoryAsync(EnergyCategory.OTHER, otherExpenseData);
    }
    ...
    private void loadWeeklyExpensesAsync() {
        CompletableFuture.supplyAsync(() -> {
            ...
            return lastSevenDays.stream().map(date -> {
                ...
                return new DataPoint(date.format(DateTimeFormatter.ofPattern("dd/MM")), total);
            }).toList();
        }).thenAcceptAsync(expenseData::setAll, uiExecutor);
    }

    private void loadMockExpensesAsync() {
        CompletableFuture.supplyAsync(() -> {
            Map<LocalDate, Double> dailyTotals = metricRepo.getAll(preferences.getActiveHouse().getId()).stream()
                    .filter(m -> m.dateTime() != null)
                    .collect(Collectors.groupingBy(m -> m.dateTime().toLocalDate(), TreeMap::new, Collectors.summingDouble(Metric::energyUsed)));

            return dailyTotals.entrySet().stream()
                    .map(e -> new DataPoint(e.getKey().format(DATE_FORMATTER), e.getValue())).toList();
        }).thenAcceptAsync(usageData::setAll, uiExecutor);
    }

    private void loadExpensesByCategoryAsync(EnergyCategory category, ObservableList<DataPoint> targetList) {
        CompletableFuture.supplyAsync(() -> {
            Map<LocalDate, Double> dailyTotals = metricRepo.getAllByCategory(preferences.getActiveHouse().getId(), category).stream()
                    .collect(Collectors.groupingBy(m -> m.dateTime().toLocalDate(), TreeMap::new, Collectors.summingDouble(Metric::energyUsed)));
    ...
    public ObservableList<DataPoint> getElectricityData() { return electricityData; }
    public ObservableList<DataPoint> getGasData() { return gasData; }
